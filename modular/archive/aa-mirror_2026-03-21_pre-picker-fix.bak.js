...
(function () {
  ...
  /* ── True Mirror Lockdown ──────────────────────────────────
     Claude: 2026-03-20 — Central lockdown for read-only mirror mode.
     On mirrorable pages (not NO_MIRROR, not SUGGESTION_KEEP),
     replaces save/submit/record/delete buttons with a styled
     read-only warning box and makes form inputs readonly.
     Network-leads (AA_MIRROR_CAN_WRITE) are NOT locked down.
     ────────────────────────────────────────────────────────── */
  ...
  function _isDark() {
    return document.documentElement.classList.contains('aa-dark');
  }

  function _makeWarningBox() {
    var warn = document.createElement('div');
    warn.className = 'aa-mirror-lockdown';
    var dark = _isDark();
    warn.style.cssText =
      'background:' + (dark ? '#252850' : '#e8eeff') + ';' +
      'border:2px solid ' + (dark ? '#4338ca' : '#a5b4fc') + ';' +
      'border-radius:12px;padding:0.85rem 1.1rem;margin:0.5rem 0;' +
      'text-align:center;max-width:400px;';
    warn.innerHTML =
      '<p style="font-weight:700;color:' + (dark ? '#a5b4fc' : '#4338ca') +
        ';margin:0 0 0.25rem 0;font-size:0.95rem;">' +
        '\uD83D\uDC41\uFE0F Mirror mode \u2014 read\u2011only' +
      '</p>' +
      '<p style="font-size:0.85rem;color:' + (dark ? '#8b8ec4' : '#5b5ea6') +
        ';margin:0;">' +
        'This action is for the student only.' +
      '</p>';
    return warn;
  }

  function _shouldSkipBtn(btn) {
    if (SKIP_IDS.indexOf(btn.id) !== -1) return true;
    /* Skip if already inside a lockdown warning */
    if (btn.closest && btn.closest('.aa-mirror-lockdown')) return true;
    /* Skip student switcher area */
  ...
  function _lockdownBtn(btn) {
    if (_shouldSkipBtn(btn)) return false;
    var text = (btn.textContent || btn.value || '').trim();
    if (!ACTION_WORDS.test(text)) return false;
    /* Already replaced */
    if (btn._aaLocked) return false;
    btn._aaLocked = true;
    var warn = _makeWarningBox();
    btn.parentNode.replaceChild(warn, btn);
    return true;
  }

  function _lockdownInputs(root) {
    root = root || document;
  ...
  function lockdownMirror() {
    /* Only lock down read-only mirror users */
    if (!window.AA_MIRROR_UID || window.AA_MIRROR_CAN_WRITE) return;
    /* NO_MIRROR pages are already blocked above */
    ...
    for (var i = 0; i < buttons.length; i++) {
      if (_lockdownBtn(buttons[i])) count++;
    }

    /* --- Make inputs read-only --- */
    _lockdownInputs();

    /* --- Intercept form submissions as safety net --- */
    var forms = document.querySelectorAll('form');
    for (var f = 0; f < forms.length; f++) {
      if (!forms[f]._aaLocked) {
        forms[f]._aaLocked = true;
        forms[f].addEventListener('submit', function (e) {
          if (window.AA_MIRROR_UID && !window.AA_MIRROR_CAN_WRITE) {
            e.preventDefault();
            e.stopPropagation();
            console.warn('[aa-mirror] Lockdown blocked form submission');
          }
        }, true);
      }
    }
    ...
    if (count > 0) {
      console.log('[aa-mirror] Lockdown: replaced ' + count + ' action button(s) with read-only warning');
    }
  }

  /* Run lockdown after DOM is ready, then use MutationObserver to
     catch dynamically-added buttons (many pages build DOM after
     Firebase data loads). */
  function initLockdown() {
    ...
    if (typeof MutationObserver !== 'undefined') {
      ...
      var observer = new MutationObserver(function () {
        ...
        _debounce = setTimeout(lockdownMirror, 300);
      });
  ...
  if (window.AA_MIRROR_UID && !window.AA_MIRROR_CAN_WRITE) {
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', function () {
        /* Small delay to let page scripts build their UI first */
