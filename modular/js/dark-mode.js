...
(function() {
  ...
  var DARK_CSS = [
    ...
    '}',
    '',
    /* Claude: 2026-03-20 — Status circle tooltip dark mode */
    'html.' + CLASS_NAME + ' #sc-tooltip {',
    '  background: rgba(30,30,30,0.96) !important;',
    '  border-color: #444 !important;',
    '  color: #e0e0e0 !important;',
    '}',
    'html.' + CLASS_NAME + ' #sc-tooltip .sc-tip-status { color: #aaa !important; }',
    'html.' + CLASS_NAME + ' #sc-tooltip .sc-tip-none { color: #777 !important; }',
    'html.' + CLASS_NAME + ' #sc-tooltip .sc-tip-study { color: #5cb85c !important; }',
    'html.' + CLASS_NAME + ' #sc-tooltip .sc-tip-study-summary {',
    ...
    try {
      ...
      window.AA.db.collection('users').doc(uid).get().then(function(doc) {
        ...
        if (data.darkMode !== undefined) {
          ...
          var localPref = null;
          try {
            localPref = localStorage.getItem(STORAGE_KEY);
          } catch (e) {
            console.warn('[AA] localStorage read failed:', e.message);
          }
